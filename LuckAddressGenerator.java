...
public class LuckAddressGenerator {
	
	public static void main(String[] args) throws GeneralSecurityException {
		boolean isPass = args.length == 1 && args[0].equals("-pass");
		boolean isRandom = args.length == 0 || args[0].equals("-r");
		boolean isPk = args.length == 1 && args[0].equals("-pk");
		if (!isPass && !isRandom && !isPk) {
			System.out.println("usage : java LuckAddressGenerator [option]");
	        		System.out.println("Options :");
	        		System.out.println("\t-pass : generate address from pass phrase (default)");
	        		System.out.println("\t-pk : generate address from private key");
	        		System.out.println("\t-r : generate random 256-bit private key");
	        		System.out.println("\t-h : print this help");
	        		return ;
		}
		if (isRandom) {
			Random rand = new Random();
			BigInteger pk = new BigInteger(256, rand);
			System.out.println("Private Key : " + pk.toString(16));
			System.out.println(
					"Luck Address: 0x" + sha3(getPub(pk.toString(16))).substring(16));
			return ;
		}
		String pass = "";
		try (Scanner scanner = new Scanner(System.in)) {
			...
			else System.out.print("Private Key : ");
			pass = scanner.nextLine();

		}
		if (isPass) {
			byte[] hash = MessageDigest.getInstance("SHA-256").digest(pass.getBytes(Charset.forName("UTF-8")));
			System.out.println("Private Key : " + padLeft(toHexString(hash).substring(2),64).replace(' ', '0'));
			System.out.println(
					"Luck Address: 0x" + sha3(getPub(toHexString(hash).substring(2))).substring(16));
		} else {
			if(pass.startsWith("0x")) pass = pass.substring(2);
            		try {
				new BigInteger(pass, 16);
			} catch (Exception e) {
				System.out.println("Wrong format, private key must be in hex format [0-9][a-f]");
				return ;
			}
			String pkLp64 = padLeft(pass,64).replace(' ', '0');
			if (!pkLp64.equals(pass))
				System.out.println("Private Key : " + pkLp64);
			System.out.println(
					"Luck Address: 0x" + sha3(getPub(pass)).substring(16));
		}
	}
	...
	final static BigInteger FieldP_2 = BigInteger.valueOf(2); // constant for scalar operations
    	...
    	public static String getPub(String privateKey) throws GeneralSecurityException {
        	String privateKeyFull = "303E020100301006072A8648CE3D020106052B8104000A042730250201010420" +
        		padLeft(privateKey,64).replace(' ', '0');
        byte[] privateKeyFullByte = hexStringToByteArray2(privateKeyFull);
        ...
        String publicKeyNativeKey = publicKeyNativeFull.substring(48, 176);
        return publicKeyNativeKey;
    }
